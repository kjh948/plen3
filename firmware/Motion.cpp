...
template <const int N> struct IF {
  enum { VALUE = 1 };
};

template <> struct IF<0> {
  enum { VALUE = 0 };
};

template <typename T> struct SIZE_SUP {
  enum { VALUE = sizeof(T) % 32 /* ExternalFs:SLOT_SIZE */ };
};

template <typename T> struct SLOT_COUNT {
  enum {
...
enum {
  ...
  SLOT_COUNT_MOTION =
      SLOT_COUNT_HEADER + SLOT_COUNT_FRAME * Header::FRAMELENGTH_MAX
};
} // namespace
...
bool Header::set() {
  ...
  if ((frame_length < FRAMELENGTH_MIN) || (frame_length > FRAMELENGTH_MAX)) {
#if DEBUG_LESS
  ...
  for (int count = 0; count < SLOT_COUNT_HEADER; count++) {
    int ret = ExternalFs::writeSlot(
        ...
        ((count == (SLOT_COUNT_HEADER - 1))
             ? ((SIZE_SUP<Header>::VALUE) ? SIZE_SUP<Header>::VALUE
...
bool Frame::set(unsigned char slot) {
  ...
  for (int count = 0; count < SLOT_COUNT_FRAME; count++) {
    int ret = ExternalFs::writeSlot(
        (static_cast<int>(slot) * SLOT_COUNT_MOTION + SLOT_COUNT_HEADER +
         index * SLOT_COUNT_FRAME + count),
        (filler + ExternalFs::SLOT_SIZE() * count),
        ((count == (SLOT_COUNT_FRAME - 1))
             ? ((SIZE_SUP<Frame>::VALUE) ? SIZE_SUP<Frame>::VALUE
                                         : ExternalFs::SLOT_SIZE())
             : ExternalFs::SLOT_SIZE()),
        fp_motion);
...
bool Frame::get(unsigned char slot) {
...
#if DEBUG_LESS
  System::debugSerial().print(F("Frame::get("));
#endif
  if (slot >= SLOT_END) {
#if DEBUG_LESS
    System::debugSerial().print(F(">>> bad argment : slot = "));
  ...
  unsigned char *filler = reinterpret_cast<unsigned char *>(this);
#if DEBUG_LESS
  System::debugSerial().print(F("SLOT_COUNT_FRAME"));
  System::debugSerial().println(SLOT_COUNT_FRAME);
#endif
  for (int count = 0; count < SLOT_COUNT_FRAME; count++) {
    int ret = ExternalFs::readSlot(
        (static_cast<int>(slot) * SLOT_COUNT_MOTION + SLOT_COUNT_HEADER +
...
#if DEBUG_LESS
    System::debugSerial().print(F("ExternalFs::readSlot ret = "));
    System::debugSerial().println(ret);
#endif
    if (ret == -1) {
#if DEBUG_LESS
...
#if DEBUG_LESS
  System::debugSerial().print(F("success"));
#endif
  return true;
}
